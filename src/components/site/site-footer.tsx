...
export function SiteFooter({ locale, content }: { locale: Locale; content: SiteFooterContent }) {
  return (
    <footer className="mt-20 bg-[#1247ff] text-white">
      <div className="mx-auto grid w-full max-w-[1400px] gap-10 px-6 py-12 lg:grid-cols-[auto_1fr_1fr_auto] lg:gap-12 lg:px-10">
        <div className="flex gap-4 text-white">
          {content.socialLinks.slice(0, 4).map((link) => {
            const label = link.label.toLowerCase();
            const isLinkedIn = label.includes("linkedin") || label.includes("in");
            return (
              <Link
                key={link.href}
                href={link.href}
        ...
        <div className="flex items-start justify-end">
          <Link
            href={`/${locale}`}
            className="inline-flex min-w-[180px] items-center justify-center bg-white px-6 py-4 text-center text-base font-bold uppercase leading-tight tracking-wide text-[#1247ff]"
          >
            {content.title}
          </Link>
        </div>
      ...
      <div className="mx-auto w-full max-w-[1400px] border-t border-white/20 px-6 py-5 text-xs leading-6 text-white/90 lg:px-10">
        <p>{content.body}</p>
        {content.copyrightText ? (
          <p className="mt-2">{content.copyrightText}</p>
        ) : null}
      </div>
    </footer>
  );
